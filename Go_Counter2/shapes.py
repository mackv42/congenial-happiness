...
for i in range(1, 22):
    ...
    imgBlur = cv2.GaussianBlur(imgResized,(7,7),1)
   ...
   # linesP = cv.HoughLinesP(imgCanny, 1, np.pi / 180, 50, None, 50, 10)
    ...
    for c in range(len(longestContours)):
        print(path + " shown")
        cv2.drawContours(imgResized, longestContours, c, (0, 255, 0), 2)
        print(str(c) + ":  length; " + str(len(longestContours[c])))
